...
async def invoke_chat(
    ...
    try:
        ...
        # 构建工作区路径
        try:
            workspace_path = settings.get_workspace_path(user_id, app_id, app_name)
        except ValueError as e:
            logger.error(f"工作区路径构建失败: {e}")
            return ChatResponse(
                code=500,
                message=f"工作区路径配置错误: {str(e)}",
                data=None,
                ai_reply=None,
                chat_id=None
            )
        ...
        agent_service = create_agent_service(
            ...
            app_id=app_id,
            app_name=app_name,
            agent_type=request.agent_type,  # 从请求中获取 agent_type，如果没有则使用默认值
